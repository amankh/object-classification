...
BATCH_SIZE = 10
NUM_ITERS = 80
LOG_DIR = 'logs/task6_1_vgg_w_mixup_w_norm'
...
def cnn_model_fn(features, labels, mode, num_classes=20):
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        #x_weight = lam.reshape(BATCH_SIZE, 1, 1, 1)
        #label_weight = lam.reshape(BATCH_SIZE, 1)
        index = np.random.permutation(BATCH_SIZE)
        x1 = input_layer
        x2 = tf.gather(input_layer, index)
        y1 = labels
        y2 = tf.gather(labels, index)

        input_layer = x1*lam + x2*(1 - lam)
        labels = y1*lam + y2*(1 - lam)
...
def main():
    ...
    if log_map == True:
        ...
        meanmapFile = open('task6_mean_map.txt','w')
