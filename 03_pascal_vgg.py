...
BATCH_SIZE = 10
NUM_ITERS = 800
LOG_DIR = '/home/ubuntu/log3/task3_1'
...
#conv layer1
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        for grad_value, grad_name in grad_list:
            if grad_value is not None:
                tf.summary.histogram("{}_grad".format(grad_name.name), grad_value)  
...
def main():
    ...
    if plot_map == True:
        ...
        plt.ion()
    if log_map == True:
        #gtmapFile = open('task3_map/gt_map.txt','w')
        #ranmapFile = open('task3_map/ran_map.txt','w')
        #evalmapFile = open('task3_map/eval_map.txt','w')
        meanmapFile = open('task3_mean_map.txt','w')
